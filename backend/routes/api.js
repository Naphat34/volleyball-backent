...
router.get('/debug/matches-schema', async (req, res) => {
  try {
    const db = require('../config/db');
    const cols = await db.query(
      "SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'matches' AND table_schema = 'public'"
    );
    res.json(cols.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
